...
for i,v in pairs(AdmIG) do
	print(v)
	cmdp[v].Chatted:Connect(function(a)
		local comsToExe = a:split(";;")
...
function useCommand.unadmin()
	if arguments[2] then
		for i,v in pairs(AdmIG) do
			if arguments[2] == AdmIG[i] then
				table.remove(AdmIG[i])
				updatesaves()
			end
		end
...
function useCommand.refreshdead()
	...
	deadRefresh.died = cmdlp.Character.Humanoid.Died:Connect(function()
		...
		deadRefresh.respawned = cmdlp.CharacterAdded:Connect(function(character)
			task.wait()
			character.HumanoidRootPart.CFrame = deadRefresh.pos
